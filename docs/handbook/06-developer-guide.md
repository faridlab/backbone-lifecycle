...
metaphor migration generate Invoice lifecycle
metaphor migration run
...
(`lifecycle` is your module name — auto-detected from the current directory when omitted, but
...
```bash
metaphor migration seed lifecycle          # run Rust seeders
metaphor migration generate-seeds lifecycle  # emit SQL seed files
```
