//! Domain policy for DisciplineRecord aggregate
//!
...
/// Domain policy for DisciplineRecord — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<DisciplineRecord>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type DisciplineRecordDomainPolicy = PermitAllPolicy<DisciplineRecord>;
