//! Domain policy for ExitInterview aggregate
//!
...
/// Domain policy for ExitInterview — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<ExitInterview>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type ExitInterviewDomainPolicy = PermitAllPolicy<ExitInterview>;
